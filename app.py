...
class recom(Resource):
    def get(self):
        ...
        Business().booked='No' #revisit
        schema = recommend()
        schema.jit = toastedmarshmallow.Jit
        merged = (rec(uid))

        return schema.dump(merged), 200 #return success
...
class afters(Resource):
    def get(self):
        ...
        schema = Business()
        schema.jit = toastedmarshmallow.Jit
        afmerged = aftersrec(lat,lon)

        return jsonify(afmerged)
...
class hist(Resource):
    def get(self):
        ...
        schema = recommend()
        schema.jit = toastedmarshmallow.Jit
        merged = c_recom.find({'user_id': ObjectId(uid)})

        return schema.dump(merged, many=True), 200 #return success

api.add_resource(recom,'/recom')
api.add_resource(afters,'/afters')
api.add_resource(hist,'/hist')
